...
n        if (serviceError) throw serviceError;
...
n      if (orderError) throw orderError;
...
n      if (paymentError) throw paymentError;
...
n      router.push(`/orders/${orderData.id}?success=true`);
    } catch (error) {
      console.error('Error:', error);
      setError(error instanceof Error ? error.message : 'Failed to place order');
    } finally {
...
n        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          {/* Checkout Form */}
          <div className="lg:col-span-2">
            <Card>
              <CardHeader>
...
n                <div className="border-t border-border pt-4 flex justify-between items-center">
                  <span className="font-semibold text-foreground">Total</span>
                  <span className="text-2xl font-bold text-primary">${service.price}</span>
                </div>

n                <p className="text-xs text-muted-foreground pt-4">
                  By placing this order, you agree to our terms and conditions
                </p>
              </CardContent>
            </Card>
